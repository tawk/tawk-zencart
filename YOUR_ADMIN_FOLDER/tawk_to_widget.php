 ...
 */
  ...
  if(isset($_GET['set'])) {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . "
          ...
          values('".TAWK_TO_PAGE_ID_FIELD."', '".$db->prepare_input(trim($_POST['page_id']))."')
          on duplicate key update configuration_value='".$db->prepare_input(trim($_POST['page_id']))."'");

    $db->Execute("insert into " . TABLE_CONFIGURATION . "
          (configuration_key, configuration_value)
          values('".TAWK_TO_WIDGET_ID_FIELD."', '".$db->prepare_input(trim($_POST['widget_id']))."')
          on duplicate key update configuration_value='".$db->prepare_input(trim($_POST['widget_id']))."'");
...
<?php
  ...
  if(strpos($tawk_widget_current_values['page_id'], '<') === 0) { //zen cart returns span with error message if configuration key can't be found
